...
async def post_usuario(usuario: UsuarioSchemaCreate, db: AsyncSession = Depends(get_session)):
    ...
    try:
        ...
        raise HTTPException(detail="Ja existe um usuário com o e-mail informado.", status_code=status.HTTP_409_CONFLICT)
...
async def get_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        raise HTTPException(detail='Usuário não encontrado', status_code=status.HTTP_200_OK)
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_session)):
    ...
    if not usuario:
        return HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Dados de acesso incorretos.', headers={''})
